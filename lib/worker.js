  ...
  if (!self._processing) {
    async.series([
        ...
        client
          ...
          .EXEC(function (err, result) {
            ...
            if (_.isEmpty(result[1])) {
              ...
              return;  // Active task not found. Stop processing.
            }
            taskId = result[1][0];
      ...
      function (cb) {
        var keys = [];
        _.each(self._watchlist, function (fn, type) {
          keys.push(builder.statusActiveKey(type));
        });
        client.WATCH(keys, cb);
      },
      function (cb) {
      ...
      function (cb) {
        client
          ...
          .PUBLISH(builder.statusUpdateChannel(), JSON.stringify({
            id: taskId,
...
Worker.prototype.watch = function (type, fn) {
  ...
  this._watchlist[type] = fn;

  this._connectPub(function (err) {
    if (err) { return console.error(err); }
  });
  this._connectSub(function (err) {
    if (err) { return console.error(err); }
  });
};
