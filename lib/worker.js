...
Worker.prototype._onTaskUpdate = function (task) {
  ...
  this._statusUpdateActiveToProcessing();
};
...
Worker.prototype._process = function (task) {
  ...
  self._watchlist[task.type](task, function (err, result) {
    if (err) {
      self._statusUpdateProcessingToFailed(task, err, result);
    } else {
      self._statusUpdateProcessingToComplete(task, result);
    }
    self._processing = false;

    process.nextTick(function () {
      self._statusUpdateActiveToProcessing();
    });
  });
};

Worker.prototype._statusUpdateActiveToProcessing = function () {
  var self = this,
      builder = self._bbq._builder,
      client,
      task,
    ...
    async.series([
      ...
      function (cb) {
        client
          ...
          .PUBLISH([
            builder.statusUpdateChannel(),
            JSON.stringify(_.assign(_.pick(task, 'id', 'type', 'status'), {
              worker: self._id
            }))
          ])
          .EXEC(function (err, result) {
...
Worker.prototype._statusUpdateProcessingToComplete = function (task, result) {
  var self = this,
      builder = self._bbq._builder;
  ...
  self._connectPub(function (err, client) {
    ...
    async.series([
      ...
      function (cb) {
        client
          .MULTI()
          .ZREM(builder.statusProcessingKey(task.type), task.id)
          .ZADD(builder.statusCompleteKey(task.type), task.score, task.id)
          .HMSET([
...
Worker.prototype._statusUpdateProcessingToFailed = function (task, err, data) {
  var self = this,
      builder = self._bbq._builder;
    ...
    async.series([
      ...
      function (cb) {
        ...
        client
          .MULTI()
          .ZREM(builder.statusProcessingKey(task.type), task.id)
          .ZADD(builder.statusFailedKey(task.type), task.score, task.id)
          .HMSET([
            builder.taskKey(task.id),
