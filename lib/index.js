...
function Barbeque(config) {
  ...
  assert((_.isString(config.password) && !_.isEmpty(config.password)) || _.isNull(config.password) || _.isUndefined(config.password),
         '"config.password" must be a non-empty string, null, or undefined.');
  assert((_.isString(config.namespace) && !_.isEmpty(config.namespace)) || _.isUndefined(config.namespace),
         '"config.namespace" must be a non-empty string or undefined.');
  ...
  if (_.isUndefined(config)) { config = {}; }
  if (_.isUndefined(config.host)) { config.host = C.DEFAULT_HOST; }
  if (_.isUndefined(config.port)) { config.port = C.DEFAULT_PORT; }
  if (_.isUndefined(config.password)) { config.password = C.DEFAULT_PASSWORD; }
  if (_.isUndefined(config.namespace)) { config.namespace = C.DEFAULT_NAMESPACE; }
  if (_.isUndefined(config.prefix)) { config.prefix = C.DEFAULT_PREFIX; }
