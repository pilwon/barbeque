...
function Barbeque(config) {
  ...
  assert((_.isNumber(config.expires) && (parseInt(config.expires, 10) === config.expires)) || _.isUndefined(config.expires),
         '"config.expires" must be an integer or undefined.');
         
  if (_.isEmpty(config)) { config = {}; }
  if (_.isNull(config.host)) { config.host = C.DEFAULT_HOST; }
  ...
  if (_.isNull(config.expires)) { config.expires = C.DEFAULT_EXPIRES; }
