...
Builder.prototype.stateActiveKey = function (type) {
  ...
  return util.format('%s:%s:%s', this._prefix, C.STATE.ACTIVE, type);
};

Builder.prototype.stateCompleteKey = function (type) {
  assert(_.isString(type) && !_.isEmpty(type), 'Type must be non-empty string.');

  return util.format('%s:%s:%s', this._prefix, C.STATE.COMPLETE, type);
};

Builder.prototype.stateFailedKey = function (type) {
  assert(_.isString(type) && !_.isEmpty(type), 'Type must be non-empty string.');

  return util.format('%s:%s:%s', this._prefix, C.STATE.FAILED, type);
};

Builder.prototype.stateInactiveKey = function (type) {
  assert(_.isString(type) && !_.isEmpty(type), 'Type must be non-empty string.');

  return util.format('%s:%s:%s', this._prefix, C.STATE.INACTIVE, type);
};

Builder.prototype.stateProcessingKey = function (type) {
  assert(_.isString(type) && !_.isEmpty(type), 'Type must be non-empty string.');

  return util.format('%s:%s:%s', this._prefix, C.STATE.PROCESSING, type);
};
...
Builder.prototype.stateUpdateChannel = function () {
