...
Builder.prototype.statusActiveKey = function (type) {
  ...
  return util.format('%s:status:%s:%s', this._prefix, C.STATUS.ACTIVE, type);
};

Builder.prototype.statusCompleteKey = function (type) {
  assert(_.isString(type) && !_.isEmpty(type), 'Type must be non-empty string.');

  return util.format('%s:status:%s:%s', this._prefix, C.STATUS.COMPLETE, type);
};

Builder.prototype.statusFailedKey = function (type) {
  assert(_.isString(type) && !_.isEmpty(type), 'Type must be non-empty string.');

  return util.format('%s:status:%s:%s', this._prefix, C.STATUS.FAILED, type);
};

Builder.prototype.statusInactiveKey = function (type) {
  assert(_.isString(type) && !_.isEmpty(type), 'Type must be non-empty string.');

  return util.format('%s:status:%s:%s', this._prefix, C.STATUS.INACTIVE, type);
};

Builder.prototype.statusProcessingKey = function (type) {
  assert(_.isString(type) && !_.isEmpty(type), 'Type must be non-empty string.');

  return util.format('%s:status:%s:%s', this._prefix, C.STATUS.PROCESSING, type);
};
...
Builder.prototype.statusUpdateChannel = function () {
  return util.format('%s:status:update', this._prefix);
};
